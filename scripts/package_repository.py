...
def included(p):
    rel=p.relative_to(R)
    if any(v in {'.git','__pycache__','node_modules','.import-v2','.changes','releases','_site'} for v in rel.parts):return False
    if p.name.startswith(('failure-','export-fixture.','smoke_v2')) or p.suffix in {'.log','.pyc'}:return False
    return rel.parts[0] in {'sdk','vendor','views','src','assets','dist','scripts','docs','tests','screenshots','.github'} or (len(rel.parts)==1 and p.name in {'README.md','LICENSE','package.json','.gitignore','requirements-assets.txt','requirements-dev.txt','duo-studio.html','duo-studio-preview.png'})
def main():
    ...
    inventory={'version':'3.0.0','files':[{'path':p.relative_to(R).as_posix(),'bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest()} for p in files]}
