...
def build():
    ...
    sources = ['icons.js','core.js','data.js','gpu.js'] + [f'apps/{a}.js' for a in APPS] + ['shell.js']
    chunks=[]
    ...
    html=(ROOT/'src/template.html').read_text().replace('/*__STYLES__*/',(ROOT/'src/styles.css').read_text()).replace('/*__SCRIPTS__*/',js)
    (ROOT/'dist').mkdir(exist_ok=True)
    out=ROOT/'dist/duo-studio.html';out.write_text(html)
    (ROOT/'dist/index.html').write_text(html)
    manifest={'version':'1.0.0','htmlBytes':out.stat().st_size,'sha256':hashlib.sha256(out.read_bytes()).hexdigest(),'assets':{k:len(v) for k,v in assets.items()},'sources':sources,'externalRuntimeDependencies':[]}
